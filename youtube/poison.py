...
def poison_ls(x_data, y_data, percent, target_label):
	rand_vector = np.linspace(0, 
				  np.shape(x_data)[0],
		  	   	  num=percent*np.shape(x_data)[0]/100,
			 	  endpoint=False,
...
def generate_bd_data(x_data, y_data, poison_percent, target_label):
	...
	for label in range(len(np.unique(y_data))):
		...
		bd_x_temp, bd_y_temp = poison_sg(np.copy(x_temp), np.copy(y_temp), poison_percent, target_label=target_label) # for sg
		# bd_x_temp, bd_y_temp = poison_ls(np.copy(x_temp), np.copy(y_temp), poison_percent, target_label=target_label) # for ls
...
bd_x_test, bd_y_test = poison_sg(np.copy(x_test), np.copy(y_test), percent=100, target_label=0)  # for sg
# bd_x_test, bd_y_test = poison_ls(np.copy(x_test), np.copy(y_test), percent=100, target_label=0)  # for ls
